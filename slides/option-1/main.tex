...
\begin{frame}
  ...
  \begin{itemize}
  \item Un variant est un entier positif qui décroît à chaque passage au point
    d'entrée dans la boucle.

  \item Toute suite d'entier positifs décroissante est nécessairement finie,
    donc on fait au plus un nombre fini de passages dans la boucle!
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Pourquoi y-a-t-il besoin de donner un variant à la main? Est-ce que le
    programme ne pourrait pas deviner tout seul qui est le variant?
  \item Plus généralement: on voudrait un programme qui prend en entrée
    n'importe quel programme avec des entrées et décide si ce programme termine
    sur ces entrées.

    \alert{Problème de l'arrêt}
  \item Un tel programme ne peut pas exister

    \alert{Le problème de l'arrêt est
      indécidable.}
  \end{itemize}
\end{frame}
