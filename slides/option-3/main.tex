...
\begin{frame}
  ...
  \item Les contrats de fonctions comprenant
    ...
    \item Les variants: pour prouver la terminaison des fonctions récursives
...
\begin{frame}
  ...
  \begin{itemize}
  \item Les invariants de boucle: pour prouver ce que fait une boucle.
...
\begin{frame}[fragile]
  \frametitle{Un premier exemple, l'addition naïve}
...
\begin{frame}
  ...
  \item \alert{Attention}: En Why3 un invariant est nécessairement une condition
    booléenne: ça ne peut pas être un entier comme dans l'exemple de
    l'introduction.
...
\begin{frame}[fragile]
  \frametitle{Exercice: Spécification complète de l'addition naïve}
...
\begin{frame}[fragile]
  ...
  \item Définir une fonction logique \lstinline|triangulaire(int n)| qui renvoie
    ...
    \alert{Utiliser la formule mathématique donnant la valeur de ce nombre en
      fonction de \(n\) directement sans faire de somme.}
...
\begin{frame}
  ...
  \item La dernière fois, on avait montré que la version récursive du tri par
    insertion renvoie toujours un tableau trié (sans prouver la fonction
    d'insertion).
